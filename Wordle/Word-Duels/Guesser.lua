-- https://www.roblox.com/games/120214928836092/Word-Duels
-- Prints Best Guesses
...
local function GetData(Plot)
	...
	for _, Row in Plot:GetChildren() do
		if not Row:IsA("Model") or not Row.Name:find("Row") then continue end

		for _, HorizontalRow in Row:GetChildren() do
			if not HorizontalRow:IsA("Model") then continue end

			for _, TextBox in HorizontalRow:GetChildren() do
				if not TextBox:IsA("Model") then continue end

				local Color  = tostring(TextBox.GuiPart.BrickColor)
				local Letter = TextBox.GuiPart.SurfaceGui.TextLabel.Text:upper()

				if Letter == "-" then continue end

				local Placement = tonumber(TextBox.Name:match("%d+"))

				if Color == "Lime green" then
					Greens = SetCharAt(Greens, Placement, Letter)

				elseif Color == "New Yeller" then
					Yellows[Placement] = Yellows[Placement] or {}
					table.insert(Yellows[Placement], Letter)

				elseif Color == "Dark stone grey" then
					Grays[Letter] = true
				end
			end
		end
...
while shared.afy and task.wait(1) do
	...
	if Hum and Hum.SeatPart then
		local Plot = Hum.SeatPart.Parent.Parent.Parent

		if Plot then
			local Possible = SolveWordle(GetData(Plot))
